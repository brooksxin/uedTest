...
/*		var cmpgn_type_cd = $(window.frames["I_market"].document).find("#CMPGN_TYPE_CD").val();
		...
		if(cmpgn_type_cd == '11'){
			...
			}else 	if(hangUpFromType ==2)//下单挂机
			{
				var order_success_flag=AiCtFrame.BusinessTracks.signInOutObj.order_sign;
				var cust_state = '2'; //成功2 失败3
				
				if(order_success_flag==1){ //1标识成功 0标识失败
					cust_state = '2';//
				}else{
					cust_state = '3';
				}
				hangupPage = "hangupOrderPage";
				$("#hangupOrderId")[0].innerText = AiCtFrame.hangUpPageType.hangUpOrderId;
				//营销下单成功，更新营销数据
				if(AiCtFrame.bussinessData.CMPGN_ID !=null && AiCtFrame.bussinessData.CMPGN_ID !="")
				{
					Util.ajax.postJsonSync(srvMap.get('updateMarketInfo'),{'CMPGN_ID':AiCtFrame.bussinessData.CMPGN_ID,'STAFF_ID':AiSoftPhone.busiAgentObj.staffId,'CUST_STS_CD':cust_state,'RESV_MOBNUM':AiCtFrame.BusinessTracks.callingObj.custMobnum,"CNTMNG_ID":AiCtFrame.BusinessTracks.contactObj.cntmngId,'RSVT_MCDS_UNIT_ID':rsvt_odr_mcds_unit_id,'BGN_CNTMNG_TIME':bgn_cntmng_time,'END_CNTMNG_TIME':end_cntmng_time,'CNTMNG_TYPE_CD':cntmng_type_cd},function(json,status){
		            if (status) {
		         		LogManager.proxyMethod.ctlogger("提交订单更新营销结果为成功2");
		            }else{
		                LogManager.proxyMethod.ctlogger(json.returnMessage+'提交订单更新营销结果异常!');
		            }
			        })
			...
			}else 	if(hangUpFromType ==5)//5：未接通
			{
				...
				if(AiCtFrame.bussinessData.CMPGN_ID !=null && AiCtFrame.bussinessData.CMPGN_ID !="")
				{
					Util.ajax.postJsonSync(srvMap.get('updateMarketInfo'),{'CMPGN_ID':AiCtFrame.bussinessData.CMPGN_ID,'STAFF_ID':AiSoftPhone.busiAgentObj.staffId,'CUST_STS_CD':0,'RESV_MOBNUM':AiCtFrame.BusinessTracks.callingObj.custMobnum,"CNTMNG_ID":0,'RSVT_MCDS_UNIT_ID':rsvt_odr_mcds_unit_id,'BGN_CNTMNG_TIME':bgn_cntmng_time,'END_CNTMNG_TIME':end_cntmng_time,'CNTMNG_TYPE_CD':cntmng_type_cd},function(json,status){
		            if (status) {
		         		LogManager.proxyMethod.ctlogger("提交订单更新营销结果为失败3！");
		            }else{
		                LogManager.proxyMethod.ctlogger(json.returnMessage+'提交订单更新营销结果异常!');
