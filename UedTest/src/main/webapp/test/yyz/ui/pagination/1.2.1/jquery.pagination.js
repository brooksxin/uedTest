...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		if (opts.gotoPage) { // 修改。zw。
			$("#"+opts.gotoPage).siblings("a.ui-button").unbind("click").bind("click",function(){
				gotoPage();
			})

			$("#"+opts.gotoPage).bind("keyup",function(e){
				var _self = $(this);
			...
			function gotoPage(){
				...
				if(newPageIndex>=totalPage){
					if(typeof result_prompt == 'function'){
						result_prompt(2,"请输入正确的页码！");
					}else{
						alert("请输入正确的页码！");
					}
					return false;
				}else{
					pageSelected(newPageIndex, event);
				}
			}
		}
