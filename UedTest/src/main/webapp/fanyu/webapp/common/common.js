...
$(function(){
	...
	*/
	if (!$('#J_login').length) {
		Util.ajax.postJsonSync(srvMap.get('curUser'), "uid=u001",function(json, status) {
			if (status) {
				window.userInfo = json;
				Util.cookie.set('postId',userInfo.bean.postId);
				if ($('#J_cUserInfo').length) 
		        	$("#J_cUserInfo").html("当前用户：<em>"+json.bean.userName+"</em>("+json.bean.postName+")");
			} else {
				Util.dialog.tips(json.returnMessage || '获取当前用户信息失败！');
			}
		});
		btnAuthority();
	};
	...
	if ($('#J_topMenu').length) {
		...
		var spaMenu = {
			"112":{
				'jsUrl':'common/market/campgnListPhoner'
			},
			"114":{
				'jsUrl':'common/market/campgnListGrLdr'	//任务管理-任务活动
			},
			"711":{
				'jsUrl':'common/system/smsList'
			},
			"525":{
				'jsUrl':'common/product/ProductGroupManage'
			},
			"1031":{
				'jsUrl':'common/questionnaire/questionaireList'  //问卷列表
			}
						
			//"1032":{
			//	'jsUrl':'common/questionnaire/questionList'  //问题列表
			//}
			// ,
			// "511":{
			// 	'jsUrl':'common/product/csProductList'
			// }
		};

		Util.ajax.postJson(srvMap.get('topMenu'),'',function(json,status){
		    if (status) {
		        Util.ajax.loadTemp('#J_topMenu',$('#T_topMenu'),json);//加载模板
		        var curFirstMenuId = Util.cookie.get('f'),
		        	curMenuId = Util.cookie.get('t');
		        menuCrumbs(curMenuId?curMenuId:curFirstMenuId);
	        	Util.cookie.set('f','1');//默认设置首页为当前高亮菜单
	        	$('#J_topMenu li a').eq(0).addClass('currentSelect');
		        var index = 0;

		        $('#J_topMenu li a').click(function(){
		        	$('#J_leftMenu').html('<div class="ui-loading"><h1 style="vertical-align:top;"><img src="../../css/images/loading.gif" alt="loading">加载中...</h1></div>');
		        	$(this).addClass('currentSelect').parents('li').siblings('li').find('a').removeClass('currentSelect');
		        	var menuId = $(this).attr('menuId');
              		Util.cookie.remove('t','/');
		        	Util.cookie.set('f',menuId);
    		        ...
    		        //left菜单
    		        if ($('#J_leftMenu').length) {
    		        	Util.ajax.postJson(srvMap.get('leftMenu'),'uid=l003&fMenu='+menuId,function(json,status){
    		        	    if (status) {
    		        	        Util.ajax.loadTemp('#J_leftMenu',$('#T_leftMenu'),json);//加载模板
    					        $('#J_leftMenu .leftMenu ul a').click(function(){
    					        	var _this = $(this),
    					        		$busi_iframe = $('#J_busi_iframe'),
    					        		$busi_div = $('#J_busi_div'),
    					        		menuId = _this.attr('menuId');
    					        	Util.cookie.set('t',menuId);
		        					_this.addClass('currentSelect').parents('li').siblings('li').find('a').removeClass('currentSelect');
		        					_this.addClass('currentSelect').parents('ul').siblings('ul').find('li a').removeClass('currentSelect');

		        					if(menuId=='121'){
		        						require(['common/market/orderDivide'], function(obj){
		        							obj.choice();
		        						});
		        					}else{
		        						//判断加载方式，iframe或div
	        							if (spaMenu[menuId]) {
			        						$busi_iframe.hide();
			        						require([spaMenu[menuId]['jsUrl']],function(fn){
				        						fn.init();
				        						var $el = fn.getTpl();
				        						$busi_div.empty().append($el).show();
			        						})
	        							}else{
			        						$busi_iframe.attr('src',_this.attr('url')).show();
			        						$busi_div.empty().hide();
	        							}
		        					}
    					        }).eq(0).trigger('click');
    		        	    }else{
    		        	        $('#J_leftMenu').html('<div style="margin:100px auto;width:55%;">'+json.returnMessage||'加载菜单失败！</div>');
    		        	    }
    		        	})
    		        };
		        });
		...
		//加载个人中心
		$('#J_usercenter').on('click',function(){
			require(['common/user/userinfo'],function(fn){
				fn();
			})
		})
	};
...
/*菜单面包屑*/
function menuCrumbs(menuId){
	if ($('.breadcrumb').length) {
		Util.ajax.postJson(srvMap.get('menuCrumbs'),'menuId='+menuId,function(json,status){
		    if (status) {
		        Util.ajax.loadTemp('.breadcrumb',$('#T_menuCrumbs'),json);//加载模板
		        var txt = $('.breadcrumb').text();
		        $('.breadcrumb').text(txt.substring(0, txt.length - 4));
		    }else{
		        //Util.dialog.tips(json.returnMessage||'查询失败，请重试！');
		    }
		})
