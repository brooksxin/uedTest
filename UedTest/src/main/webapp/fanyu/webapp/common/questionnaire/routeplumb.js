getdata = function (npsjson) {
    ...
    for (var i = 0; i < questions.length; i++) {
        ...
        if (questions[i].bean.TGT_ROUTE_ID
            && questions[i].bean.TGT_ROUTE_ID != "") {
            var qline = {};
            ...
            if (questions[i].bean.QU_TYPE_CD_NAME == "单选题") {
                var options = questions[i].beans;
                for (var j = 0; j < options.length; j++) {
                    if (options[j].TGT_ROUTE_ID
                        && options[j].TGT_ROUTE_ID != "") {
                        var sline = {};
            ...
            if (questions[i].bean.QU_TYPE_CD_NAME == "判断题") {
                var options = questions[i].beans;
                for (var j = 0; j < options.length; j++) {
                    if (options[j].TGT_ROUTE_ID
                        && options[j].TGT_ROUTE_ID != "") {
...
createDiv = function (nodeSize, nodesList) {
    ...
    for (var i = 0; i < nodesList.length; i++) {
        if (nodesList[i].qu_type_cd == "0B") {
            divnodes += '<div class="node" id="' + nodesList[i].qu_id
                + '" style="width: ' + nodeSize[0] + 'px; height: '
                + nodeSize[1] + 'px;"><em><strong>'
                + nodesList[i].qu_type_nm + '</strong></em></br>'
                + nodesList[i].qu_nm + '</div>';
        } else if (nodesList[i].qu_type_cd == "0E") {

            divnodes += '<div class="node" id="' + nodesList[i].qu_id
                + '" style="width: ' + nodeSize[0] + 'px; height: '
                + nodeSize[1] + 'px;"><em><strong>' + qcount + "."
                + nodesList[i].qu_type_nm + '</strong></em></br>'
                + nodesList[i].qu_nm + '</div>';
            qcount++;
        } else {
            divnodes += '<div class="node" id="' + nodesList[i].qu_id
                + '" style="width: ' + nodeSize[0] + 'px; height: '
                + nodeSize[1] + 'px;"><em><strong>' + ccount + "."
                + nodesList[i].qu_type_nm + '</strong></em></br>'
                + nodesList[i].qu_nm + '</div>';
            ccount++;
        }
...
jsPlumb.ready(function () {
    ...
    var htmlobj = $.ajax({
        type:'get',
        url: "/ecp/front/sh/questionaire!index?uid=qoro022&QNR_ID=" + GetQueryString("QNR_ID"),
        async: false,
        datatype:'text',
    ...
    // 创建节点
    var map = getdata(jsondata.object);
    createNodes([200, 50], [100, 60], [120, 120], 20,
        map.nodelist);
    ...
    instance.batch(function () {
        for (var i = 0; i < map.nodelist.length; i++) {
            _addEndpoints(map.nodelist[i].qu_id, ["RightMiddle",
                "BottomCenter"], ["TopCenter", "LeftMiddle"]);
        }
        ...
        instance.draggable(
            jsPlumb.getSelector(".flowchart-demo .node"), {
                grid: [20, 20]
            });
        ...
        for (var i = 0; i < map.linelist.length; i++) {
            instance.connect({
                uuids: [map.linelist[i].qu_id + "RightMiddle",
                    map.linelist[i].next + "LeftMiddle"],
                editable: true
            });
        }

        //
        // listen for clicks on connections, and offer to delete
        // connections on
        // click.
        //
        ...
        instance.bind("connectionMoved", function (params) {
            console.log("connection " + params.connection.id
                + " was moved");
        });
    });
