define(['handlebars', 'dialog', 'text!tpl/market/againdivideTaskSys.tpl', 'text!tpl/market/againdivideTaskSys-list.tpl', 'jquery', 'underscore', 'jquery.countdown'], function(Handlebars, dialog, tpl, listTpl, $){
	...
	var listInit = function(){
		...
		//获取任务数量
		var params={"uid":"mc003","CMPGN_ID":CMPGN_ID,"type":"0"};
		Util.ajax.postJsonSync(srvMap.get('getcount'), params, function(json, status) {
			if (status) {
				...
				//查询部门列表
				var param={'uid':'u003'};
				Util.ajax.postJsonSync(srvMap.get('stafflist'),param, function(json, status) {
					if (status) {
